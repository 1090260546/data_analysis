...
import {
  EmptySchedule,
  EmptyWeChatWorkSchedule
} from 'app/containers/Schedule/constants'
import { mockAnonymousAction } from 'test/utils/fixtures'
import { mockStore } from './fixtures'

describe('projectsReducer', () => {
  const { scheduleId, schedules, schedule, jobType, mails, dashboard } =
    mockStore
  let state
  beforeEach(() => {
    state = initialState
  ...
  it('should handle the  addScheduleFail action correctly', () => {
    ...
    expect(reducer(state, actions.addScheduleFail())).toEqual(expectedResult)
    expect(reducer(state, actions.editScheduleFail())).toEqual(expectedResult)
  })
  ...
  it('should handle the  changeScheduleJobType action correctly', () => {
    const expectedResult = produce(state, (draft) => {
      draft.editingSchedule =
        jobType === 'email' ? EmptySchedule : EmptyWeChatWorkSchedule
    })
  ...
  it('should handle the  portalDashboardsLoaded action correctly', () => {
    ...
    expect(
      reducer(state, actions.portalDashboardsLoaded(scheduleId, [dashboard]))
    ).toEqual(expectedResult)
  })
})
