...
const Projects: React.FC<IProjectsProps & RouteComponentWithParams> =
  React.memo(
    ({
      projects,
      onLoadProjects,
      ...
      const loginUserId = useMemo(() => loginUser && loginUser.id, [loginUser])
      ...
      const getProjectsBySearch = useMemo(() => {
        ...
        function filterByKeyword(arr: IProject[]) {
          return (
            ...
            arr.filter(
              (pro: IProject) =>
                pro.name.toUpperCase().indexOf(searchKeywords.toUpperCase()) >
                -1
            )
          )
        }
